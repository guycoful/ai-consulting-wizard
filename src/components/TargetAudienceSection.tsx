      ...
      <div ref={ref} className={`animate-scroll-in ${isVisible ? 'visible' : ''} container mx-auto px-4 max-w-4xl`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {targetPoints.map((point, index) => (
            <div key={index} className="flex items-start gap-4 bg-navy-dark p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 border-r-4 border-gold">
              ...
              <p className="text-lg font-heebo text-gray-300 leading-relaxed">
                {point}
              </p>
            </div>
          ))}
